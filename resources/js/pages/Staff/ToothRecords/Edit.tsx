...
export default function StaffToothRecordsEditPage({ toothRecordId }: Readonly<{ toothRecordId: string }>) {
    ...
    useEffect(() => {
        const fetchRecord = async () => {
            try {
                setLoading(true);
                setError(null);
                const data = await apiStaff.getToothRecord(Number(toothRecordId));
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            setSaving(true);
            setError(null);
            await apiStaff.updateToothRecord(record.id, {
                service: form.service,
                date_done: form.date_done,
                notes: form.notes
            });
            router.visit(`/staff/tooth-records/${record.id}`);
        } catch (err: any) {
    ...
    return (
        <AppLayout>
            <Head title="Edit Service Transaction - DentalinkCare" />
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-8">
                        ...
                        {!loading && record && (
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label htmlFor="staff-tooth-record-service" className="block text-sm font-medium text-gray-700 mb-2">
                                            Service
                                        </label>
                                        <input
                                            id="staff-tooth-record-service"
                                            type="text"
                                            value={form.service}
                                    ...
                                    <div>
                                        <label htmlFor="staff-tooth-record-date" className="block text-sm font-medium text-gray-700 mb-2">
                                            Date Done
                                        </label>
                                        <input
                                            id="staff-tooth-record-date"
                                            type="date"
                                ...
                                <div>
                                    <label htmlFor="staff-tooth-record-notes" className="block text-sm font-medium text-gray-700 mb-2">
                                        Notes
                                    </label>
                                    <textarea
                                        id="staff-tooth-record-notes"
                                        value={form.notes}
                                ...
                                <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                                    <Link
                                        href={`/staff/tooth-records/${record.id}`}
                                    ...
                                    <button
                                        type="submit"
                                        disabled={saving}
                                        className="px-5 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-60"
                                    >
                                        {saving ? (
