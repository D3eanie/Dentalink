...
export default function AdminServicesPage() {
    ...
    const fetchServices = async () => {
        try {
            setLoading(true);
            const response = await apiStaff.getServices(filters);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditMode && selectedService) {
                await apiStaff.updateService(selectedService.id, serviceData);
                // apiStaff.updateService shows a success Swal internally
            } else {
                await apiStaff.createService(serviceData);
                // apiStaff.createService shows a success Swal internally
            }
            ...
            if (error.response?.data?.errors) {
                ...
                // apiStaff.showErrorToast('Please correct the form errors.');
            } else {
                // apiStaff methods already show an error Swal on API failure
                console.error(`Failed to ${isEditMode ? 'update' : 'create'} service:`, error);
                // The error is logged here, but the user is notified via the Swal toast from apiStaff
            }
        }
    };
    ...
    const handleDelete = async (service: Service) => {
        try {
            // apiStaff.deleteService handles the Swal confirmation dialog, the DELETE API call,
            // and shows the success/error Swal messages internally. This prevents the ReferenceError in Index.tsx.
            const result = await apiStaff.deleteService(service.id, service.name);
            ...
            // The error is logged here. The user is notified via the Swal error toast from apiStaff
