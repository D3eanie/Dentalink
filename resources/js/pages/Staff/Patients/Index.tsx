...
export default function AdminPatientsPage() {
    ...
    const fetchPatients = async () => {
        try {
            setLoading(true);
            const response = await apiStaff.getPatients({
                ...filters,
                page: currentPage
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditMode && selectedPatient) {
                ...
                await apiStaff.updateUser(selectedPatient.id, submissionData);
            } else {
                console.log('Creating new patient');
                await apiStaff.createUser(submissionData);
            }
    ...
    const handleDelete = async (patient: Patient) => {
        try {
            ...
            const result = await apiStaff.deleteUser(patient.id, fullName);
    ...
    const handleExport = async () => {
        try {
            ...
            await apiStaff.exportPatients(startDate, endDate);
        } catch (error) {
                                            ...
                                            <tr key={patient.id} className="hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end items-center gap-2">
                                                        <button
                                                            onClick={() => window.location.href = `/staff/tooth-records?patient_id=${patient.id}`}
