  ...
  const fetchBillingRecords = async () => {
    try {
      ...
      const [recordsRes, summaryRes] = await Promise.all([
        apiPatient.getMyBillingRecords(filters),
        apiPatient.getFinancialSummary(filters)
      ]);

      // Extract records
      let recordsList = [];
      if (recordsRes?.data) {
        recordsList = Array.isArray(recordsRes.data) ? recordsRes.data :
      ...
      let summaryData = {};
      if (summaryRes?.data) {
        summaryData = summaryRes.data.summary || summaryRes.data || summaryRes;
      } else {
        summaryData = summaryRes?.summary || summaryRes || {};
      }

      if (summaryData && Object.keys(summaryData).length > 0) {
        setSummary({
          total_amount: Number(summaryData.total_amount || 0),
          total_paid: Number(summaryData.total_paid || 0),
          total_pending: Number(summaryData.total_pending || 0),
          total_overdue: Number(summaryData.total_overdue || 0)
        });
      } else {
        // Calculate summary from records
        const totalAmount = recordsList.reduce((sum: number, r: BillingRecord) => sum + r.amount, 0);
        const totalPaid = mappedRecords
          .filter((r: BillingRecord) => r.payment_status === 'paid')
          .reduce((sum: number, r: BillingRecord) => sum + r.amount, 0);
        const totalPending = mappedRecords
          .filter((r: BillingRecord) => ['pending', 'partial'].includes(r.payment_status))
          .reduce((sum: number, r: BillingRecord) => sum + (r.balance ?? r.amount), 0);
        const totalOverdue = mappedRecords
