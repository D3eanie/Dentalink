...
const breadcrumbs = [
    ...
    { title: 'Service Transactions', href: '/admin/tooth-records' }
];

export default function AdminToothRecordsPage() {
    // Get URL parameters
    ...
    const fetchToothRecords = async () => {
        try {
            ...
            console.log('[ToothRecords] apiAdmin methods:', Object.getOwnPropertyNames(Object.getPrototypeOf(apiAdmin)).filter(m => !m.startsWith('_')));
            console.log('[ToothRecords] Fetching with filters:', filterParams);
            const response = await apiAdmin.getToothRecords(filterParams);
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('Are you sure you want to delete this service transaction?')) {
            try {
                await apiAdmin.deleteToothRecord(id);
                fetchToothRecords();
            } catch (error) {
    ...
    const handleMarkReviewed = async (id: number) => {
        try {
            await apiAdmin.markToothRecordReviewed(id);
            fetchToothRecords();
        } catch (error) {
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link
                                href="/admin/tooth-records/create"
                            ...
                            <div className="flex items-center gap-2">
                                {patientId && (
                                    <button
                                        onClick={() => {
                                            setPatientId('');
                                            window.history.pushState({}, '', '/admin/tooth-records');
                                        }}
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-6 py-4 text-sm">
                                                            <div className="flex items-center justify-center gap-2">
                                                                <Link
                                                                    href={`/admin/tooth-records/${group.records[0].id}`}
