...
const computeStatsFromAppointments = (appointments: Appointment[]) => {
  console.log('Computing stats from appointments:', appointments.length);
  ...
  appointments.forEach((appointment) => {
    ...
    console.log('Checking appointment:', appointment.appointment_date, '-> normalized:', apptDate, 'status:', appointment.status);

    if (apptDate.getTime() === today.getTime()) {
      todayCount += 1;
      console.log('  -> Counts as TODAY');
    }
    if (apptDate >= startOfWeek && apptDate <= endOfWeek) {
      weekCount += 1;
      console.log('  -> Counts as THIS WEEK');
    }
...
export default function AppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await apiAdmin.getAppointments(cleanedFilters);
      ...
      console.log('Fetched appointments list:', appointmentsList.length, 'appointments');
      setAppointments(appointmentsList);

      // Compute stats from appointments list
      console.log('About to compute stats from', appointmentsList.length, 'appointments');
      const computedStats = computeStatsFromAppointments(appointmentsList);
      console.log('Setting stats to:', computedStats);
  ...
  const fetchDropdownData = async () => {
    try {
      // Fallback implementation if getUsersByRole doesn't exist
      const getUsersByRole = async (role: string) => {
        if (typeof apiAdmin.getUsersByRole === 'function') {
          return apiAdmin.getUsersByRole(role);
        }
      ...
      if (Array.isArray(doctorsRes)) {
        doctorsList = doctorsRes;
      } else if (doctorsRes?.data) {
        doctorsList = Array.isArray(doctorsRes.data) ? doctorsRes.data :
                     (doctorsRes.data.data || doctorsRes.data.users || []);
      } else if (doctorsRes?.users) {
        doctorsList = doctorsRes.users;
      }
      ...
      if (Array.isArray(patientsRes)) {
        ...
        patientsList = Array.isArray(patientsRes.data) ? patientsRes.data :
                      (patientsRes.data.data || patientsRes.data.users || []);
      } else if (patientsRes?.users) {
        patientsList = patientsRes.users;
      }
      ...
      if (Array.isArray(servicesRes)) {
        ...
        servicesList = Array.isArray(servicesRes.data) ? servicesRes.data :
  ...
  useEffect(() => {
    console.log('Stats state updated to:', stats);
  }, [stats]);
  ...
  const handleDelete = async (appointment: Appointment) => {
    try {
      await apiAdmin.deleteAppointment(appointment.id);
      await fetchAppointments();
    } catch (error) {
  ...
  const handleEdit = async (appointment: Appointment) => {
    ...
    // FIXED: Format the time to HH:MM format
  ...
  const handleCheckIn = async (appointment: Appointment) => {
    ...
    window.location.href = `/admin/appointments/${appointment.id}/tooth-records`;
  };
  ...
  const handleComplete = async (appointment: Appointment) => {
    try {
      ...
      setTimeout(() => {
        window.location.href = `/admin/financial?appointment_id=${appointment.id}`;
      }, 2500);
    } catch (error) {
  ...
  const handleQuickConfirm = async (appointment: Appointment) => {
    try {
      await apiAdmin.quickConfirmAppointment(appointment.id);
      await fetchAppointments();
    } catch (error) {
  ...
  const submitPaymentConfirmation = async () => {
    ...
    try {
      ...
      await apiAdmin.confirmAppointmentPayment(
  ...
  const handleSubmitPartialPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiAdmin.createPartialPaymentFollowUp(parentRecord.id, {
